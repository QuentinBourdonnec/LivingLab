...
EchoClient::EchoClient(const QUrl &url, QObject *parent) : QObject(parent), m_url(url)
{
    ...
    connect(&m_webSocket, &QWebSocket::connected, this, &EchoClient::onConnected);//On se connecte au serveur
    connect(&m_webSocket, &QWebSocket::disconnected, this, &EchoClient::closed);//On ferme la connexion
...
int EchoClient::binaryToDecimal(int num)
{
    int decimal_val = 0;
    int base = 0;
    int reste;
...
void EchoClient::DatabaseConnexion(int var1, int var2, int var3, int var4, int var5, QString var6, QString var7)
{
    ...
    pstmt = con->prepareStatement("INSERT INTO VALUE (DTIME,ID_SENSOR, NUM, ID_ROOM) VALUES (?, 1, ?, 1)");
    pstmt->setDateTime(1, var6.mid(0,19).toStdString());
    pstmt->setInt(2, var1);
    pstmt->executeUpdate();

    //On insère l'humidite dans la base de donnée
    pstmt = con->prepareStatement("INSERT INTO VALUE (DTIME,ID_SENSOR, NUM, ID_ROOM) VALUES (?, 2, ?, 1)");
    pstmt->setDateTime(1, var6.mid(0,19).toStdString());
    pstmt->setInt(2, var2);
    pstmt->executeUpdate();

    //On insère la température dans la base de donnée
    pstmt = con->prepareStatement("INSERT INTO VALUE (DTIME,ID_SENSOR, NUM, ID_ROOM) VALUES (?, 3, ?,1)");
    pstmt->setDateTime(1, var6.mid(0,19).toStdString());
    pstmt->setInt(2, var3);
    pstmt->executeUpdate();

    //On insère la chute dans la base de donnée
    pstmt = con->prepareStatement("INSERT INTO VALUE (DTIME,ID_SENSOR, NUM, ID_ROOM) VALUES (?, 4, ?,1)");
    pstmt->setDateTime(1, var6.mid(0,19).toStdString());
    pstmt->setInt(2, var4);
    pstmt->executeUpdate();

    //On insère le four dans la base de donnée
    pstmt = con->prepareStatement("INSERT INTO VALUE (DTIME,ID_SENSOR, NUM, ID_ROOM) VALUES (?, 5, ?,1)");
    pstmt->setDateTime(1, var6.mid(0,19).toStdString());
    pstmt->setInt(2, var5);
    pstmt->executeUpdate();
...
void EchoClient::rec_seuil()
{
    ...
    //On appel les fonctions pour réaliser les différentes modifications
...
void EchoClient::verif_four()
{
    ...
    //récupération de la valeur actuelle du four
    pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 5 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    res->last();
    ...
    if(valeur_moment == 1)
    {
        ...
        //Ne fonctionne pas
       /* pstmt = con->prepareStatement("SELECT DTIME FROM VALUE WHERE ID_SENSOR = 5 AND ID_ROOM= 1");
        res = pstmt->executeQuery();
        res->last();
        ...
        pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 5 AND ID_ROOM=1 AND DTIME = ?");
        ...
        if(valeur_deuxh == 0)
        {
            ...
            pstmt =con->prepareStatement("SELECT MAIL_USER FROM USER_ROOM WHERE ID_ROOM= 1");
            res = pstmt->executeQuery();
            res->last();
            ...
            pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte declenchee a cause du four au LivingLab de Brest', 1, ?)");
            pstmt->setString(1, QString(user.c_str()).toStdString());
            pstmt->executeUpdate();
        }*/
...
void EchoClient::verif_chute()
{
    ...
    pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 4 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    res->last();
    ...
    if(valeur == 1)
    {
        ...
        //Contenant du sms "Attention !! L'utilisateur du LivingLab de Brest vient de chuter, il est probable que cette chute ait des répercutions sur la santé de cette personne, merci de faire le nécessaire pour vous assurez qu'il va bien. Cordialement, service technique du LivingLab."
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte SMS declenchee a cause de chute au livinglab de brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }
...
void EchoClient::verif_hum(int a, int b)
{
    Smtp *smtp = new Smtp("elevecir2", "Cir2Projet", "smtp.isen-ouest.fr");
    int valeur;
    sql::Driver *driver;
    ...
    pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 3 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    res->last();
    valeur = res->getInt("NUM");
    ...
    if(valeur < a || valeur > b)
    {
       ...
       // smtp->sendMail("quentin.bourdonnec@isen-bretagne.fr", "quentin.bourdonnec@laposte.net" , "Alerte d'humidite au LivingLab de Brest", "Attention !! Le taux d'humidite est anormalement eleve au sein du LivingLab, cette humidite peut être signe d'inondation et peut causer de nombreux dommages. Merci de faire le nécessaire et vous assurer que l'utilisateur est en bonne sante car ce taux d'humite important peut causer des troubles respiratoires. Cordialement, service technique du LivingLab.");
        smtp->sendMail("quentin.bourdonnec@isen-bretagne.fr", QString(mail.c_str()) , "Alerte d'humidite au LivingLab de Brest", "Attention !! Le taux d'humidite est anormalement eleve au sein du LivingLab, cette humidite peut être signe d'inondation et peut causer de nombreux dommages. Merci de faire le nécessaire et vous assurer que l'utilisateur est en bonne sante car ce taux d'humite important peut causer des troubles respiratoires. Cordialement, service technique du LivingLab.");
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte mail declenchee a cause de humidite en dehors des seuils au living lab de Brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }
...
void EchoClient::verif_temp(int a, int b)
{
    Smtp *smtp = new Smtp("elevecir2", "Cir2Projet", "smtp.isen-ouest.fr");
    int valeur_mom;
    sql::Driver *driver;
    ...
    pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 2 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    res->last();
    ...
    if(valeur_mom < a || valeur_mom > b)
    {
        ...
        smtp->sendMail("quentin.bourdonnec@isen-bretagne.fr", QString(mail.c_str()) , "Alerte de temperature au LivingLab de Brest, en dehors des seuils habituels", "Attention !! La temperature est anormalement eleve au sein du LivingLab. Cette  temperature peut etre signe de depart d'incendie et peut causer de nombreux dommages. Merci de faire le necessaire et vous assurer que l'utilisateur est en bonne sante car cette forte temperature peut causer des troubles respiratoires ainsi que d'autres problemes. Verifiez que l'ensemble des appareils electromenagers sont bien eteint. Cordialement, service technique du LivingLab.");
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte mail de temperature en dehors des seuils au LivingLab de Brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }
    ...
    //Ne fonctionne pas
    //Par récupération de l'heure
    /*pstmt = con->prepareStatement("SELECT DTIME FROM VALUE WHERE ID_SENSOR = 2 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    res->last();
    std::string time = res->getString("DTIME");
    QDateTime date;
    ...
    pstm = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR = 2 AND ID_ROOM=1 AND DTIME = ?");
    pstmt->setString(1, ntime.toStdString());
    res = pstmt->executeQuery();
    res->last();
    ...
    if(fabs(diff) > 3)
    {
        ...
        smtp->sendMail("quentin.bourdonnec@isen-bretagne.fr", QString(mail.c_str()) , "Alerte de temperature au LivingLab de Brest, augmentation en peut de temps" ,"Attention !! La temperature vient d'augmenter de 3°C au cours de ces 5 dernieres minutes, et est anormalement eleve au sein du LivingLab. Cette  temperature peut etre signe de depart d'incendie et peut causer de nombreux dommages. Merci de faire le necessaire et vous assurer que l'utilisateur est en bonne sante car cette forte temperature peut causer des troubles respiratoires ainsi que d'autres problemes. Verifiez que l'ensemble des appareils electromenagers sont bien eteint. Cordialement, service technique du LivingLab.");
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte mail de temperature en hausse au LivingLab de Brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }*/
...
void EchoClient::verif_CO2(int a, int b)
{
    Smtp *smtp = new Smtp("elevecir2", "Cir2Projet", "smtp.isen-ouest.fr");
    int valeur;
    sql::Driver *driver;
    ...
    pstmt = con->prepareStatement("SELECT NUM FROM VALUE WHERE ID_SENSOR =1 AND ID_ROOM= 1");
    res = pstmt->executeQuery();
    ...
    if(valeur > a)
    {
        ...
        smtp->sendMail("quentin.bourdonnec@isen-bretagne.fr", QString(mail.c_str()) , "Alerte de CO2 au LivingLab de Brest","Attention !! Le taux de CO2 est anormalement eleve au sein du LivingLab, ce taux peut etre signe de depart d'incendie et peut causer de nombreux dommages. Merci de faire le necessaire et vous assurer que l'utilisateur est en bonne sante car ce taux de CO2 important peut causer des troubles respiratoires. Cordialement, service technique du LivingLab.");
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte mail declenchee a cause de CO2 au dessus du seuil maximum au LivingLab de Brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }
    ...
    if(valeur > b)
    {
        ...
        pstmt = con->prepareStatement("INSERT INTO ALERT (DETAIL, ID_ROOM, MAIL_USER) VALUES ('Alerte SMS declenchee a cause de CO2 au dessus du seuil maximum au LivingLab de Brest', 1, ?)");
        pstmt->setString(1, QString(user.c_str()).toStdString());
        pstmt->executeUpdate();
    }
